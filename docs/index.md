...
1. **多账户支持**：灵活映射不同支付工具到对应会计科目
1. **交易状态处理**：支持成功、失败、退款等多种状态
1. **元数据提取**：自动从账单头部提取账户信息和日期范围
1. **重复检测**：智能识别并处理重复交易
1. **归档管理**：支持原始文件的自动归档和整理
...
1. **Few-shot 学习**：基于相似历史交易提供上下文
1. **向量相似度搜索**：使用 `usearch` 库快速查找相似交易
1. **智能缓存**：`diskcache` 提高重复查询性能
